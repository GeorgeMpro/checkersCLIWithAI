...
from utils import extract_index_from_cell_name, get_cell_row_from_name, is_valid_cell_name
...
class MoveHandler:
    ...
    def is_valid_distance(source_cell: Cell, target_cell: Cell):
        # a_ij source cell
        i, j = extract_index_from_cell_name(source_cell)
        # a_kl target cell
        k, l = extract_index_from_cell_name(target_cell)
        row_distance = i - k
        col_distance = j - l
    ...
    def get_destination_for_capture(source_cell: Cell, target_cell: Cell) -> str:
        row_src, col_src = extract_index_from_cell_name(source_cell)
        row_trg, col_trg = extract_index_from_cell_name(target_cell)
